...
export function autoWired<T>(
...
): ((target: any, propertyKey: string | symbol, parameterIndex?: number) => void) | void {
  if (typeof token === 'function') {
    ...
    if (type === undefined) {
      ...
      throw new Error(`failed to get design type of ${(token as any).constructor.name}'s ${String(propertyKey)}`);
    }
