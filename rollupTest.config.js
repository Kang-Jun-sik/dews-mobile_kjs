...
const dewsSamplePlugin = () => {
  ...
  return {
    ...
    buildStart(inputOptions) {
      inputOptions.input = globby.sync(inputOptions.input);
    },
    generateBundle() {
      // index.html 생성
...
export default [
  {
    ...
    output: {
      ...
      entryFileNames: filePath => {
        let path = filePath.facadeModuleId.replace(/\//g, `\\`);
        path = path.split(__dirname.replace(/\//g, `\\`))[1];
        path = path.split('\\');
        path = path.slice(2, path.length);
        path = path.join('/');
        const entryPath = path.split('.')[0];
        return `src/${entryPath}.js`;
      },
      chunkFileNames: 'chunkFile.js',
      format: 'es',
    },
  },
];
